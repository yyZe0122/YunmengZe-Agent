...
- 状态：Accepted
- 日期：2026-07-13
- 更新：2026-09-03（`runs.child_kind` / `child_tools` migration 029，ADR-039 `task_id` resume）
...
Core migration runner 统一拥有 schema 版本。Migration 013 删除旧 Module Registry、消费位点和 Evolution Activation 遗留表；015–029 为后续追加。历史 migration 序号保留，避免破坏已经存在的 `core.db` 升级路径。
