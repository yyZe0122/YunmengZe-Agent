...
**下一优先：** TUI-ink + IDE-launcher V0–V3 已树内，等发 **v0.4.0**。原生聊天 / Marketplace（V4–V5）与 O5–O6 / H2 / M* 等用户再提。
...
V0–V3 已树内：`extensions/vscode` 启动 TUI；`make vscode` 本机打包；发版脚本 / Actions 在 goreleaser 之后挂 `ymz-vscode_{version}.vsix`。用户说明：[`docs/wiki/vscode.md`](../wiki/vscode.md)。架构：[ADR-054](../wiki/adr/054-vscode-terminal-launcher.md)。
